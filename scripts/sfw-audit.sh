...
# Use \b word-boundary anchors to avoid false positives (e.g. "class" matching "ass")
SANITIZE_PATTERNS=(
  '\b(fuck|shit|damn|ass|bitch|crap)\b'  # profanity → [REDACTED]
)
...
sanitize_file() {
  local file="$1"
  for pat in "${SANITIZE_PATTERNS[@]}"; do
    # Replace matched words with [REDACTED] — word-boundary aware, case insensitive
    perl -i -pe "s/$pat/[REDACTED]/gi" "$file" 2>/dev/null || true
  done
}
...
REPORT
  ...
  # Write newline-delimited file lists for reliable Python parsing
  printf '%s\n' "${PASS_LIST[@]:-}"     > /tmp/sfw_pass.txt
  printf '%s\n' "${SANITIZE_LIST[@]:-}" > /tmp/sfw_sanitize.txt
  printf '%s\n' "${EXCLUDE_LIST[@]:-}"  > /tmp/sfw_exclude.txt
  printf '%s\n' "${REVIEW_LIST[@]:-}"   > /tmp/sfw_review.txt
...
manifest = {
    ...
    "diff_base": "$DIFF_BASE",
    "mece": {
        "PASS":     read_list("/tmp/sfw_pass.txt"),
        "SANITIZE": read_list("/tmp/sfw_sanitize.txt"),
        "EXCLUDE":  read_list("/tmp/sfw_exclude.txt"),
        "REVIEW":   read_list("/tmp/sfw_review.txt"),
    },
    "policy": {
        "exclude_patterns": [
            "ITAR/export-controlled",
        ...
        "sanitize_patterns": ["profanity (word-boundary matched)"],
    },
